...
def start(message):
    ...
    if db.is_user_exists(user_id):
        ...
        mess = bot.send_message(user_id,
                                'Здравствуйте. Мы общественная организация, которая поможет Вам найти основную работу или подработку. Предлагаем Вам заполнить небольную анкету',
                                reply_markup=markup)
...
def process_photo_step(message):
    ...
    try:
        ...
        mess = bot.send_message(user_id, 'Ошибка загрузки! Попробуйте отправить другую фотографию',
...
def process_sex_step(message):
    ...
    if message.text in ['Мужской', 'Женский']:
        ...
        mess = bot.send_message(user_id,
                                'Некорректный ввод. Пожалуйста, выберите один из представленных вариантов ответа')
        bot.register_next_step_handler(mess, process_sex_step)
...
def process_born_step(message):
    ...
    try:
        ...
        mess = bot.send_message(user_id, 'Введите Ваш контактный номер телефона (формат 89995550011)',
                                reply_markup=hide_board)
        bot.register_next_step_handler(mess, process_phone_step)
    except ValueError:
        mess = bot.send_message(user_id,
                                'Некорректный ввод. Пожалуйства введите дату в соответствии с шаблоном (дд.мм.гггг)')
        bot.register_next_step_handler(mess, process_born_step)
...
def process_phone_step(message):
    ...
    if bool(re.match(r"^\d+$", message.text)):
        ...
        mess = bot.send_message(user_id,
                                'Неверный формат, пожалуйства введите номер в соответствии с шаблоном (формат: 85551116699)')
        bot.register_next_step_handler(mess, process_phone_step)
...
def process_min_salary_step(message):
    ...
    if bool(re.match(r"^\d+$", message.text)):
        ...
        mess = bot.send_message(user_id,
                                'Готовы ли Вы выполнять тяжелую работу (копать, ломать, строить, быть грузчиком)?',
                                reply_markup=markup)
...
def process_hardwork_step(message):
    ...
    if message.text in ['Да', 'Нет']:
        ...
        markup.add(btn_yes, btn_no)
        mess = bot.send_message(user_id,
                                'Готовы ли Вы выполнять работу в сфере обслуживания (уборка, няня, выгул собак ...)?',
                                reply_markup=markup)
...
def process_midwork_step(message):
    ...
    if message.text in ['Да', 'Нет']:
        ...
        markup.add(btn_yes, btn_no)
        mess = bot.send_message(user_id,
                                'Готовы ли Вы выполнять творческую работу (аниматорство, ведение соцсетей, фото/видеосъемка и.т.д.)?',
                                reply_markup=markup)
...
def process_artwork_step(message):
    ...
    if message.text in ['Да', 'Нет']:
        ...
        mess = bot.send_message(user_id,
                                'Напишите пожалуйста, какие специализированные виды работ или услуг Вы выполняли и какими навыками обладаете?',
                                reply_markup=hide_board)
...
def process_addwork_step(message):
    ...
    mess = bot.send_message(user_id,
                            'Какие инструменты, оборудование или аппаратура у Вас имеется (компьютер, дрель, фотоаппарат ...)?',
                            reply_markup=hide_board)
...
def process_tools_step(message):
    ...
    mess = bot.send_message(user_id, 'Выберите Ваше транспортное средство. При его отсутствии выберите Нет',
                            reply_markup=markup)
...
def process_car_step(message):
    ...
    markup.add(btn_pass)
    mess = bot.send_message(user_id,
                            'Почти закончили! Укажите пожалуйста Ваш адрес проживания (район, улица, дом). Это поможет нам быстрее найти работу поблизости',
                            reply_markup=markup)
...
def process_save_step(message):
    ...
    if message.text == 'Сохранить':
        ...
        markup.add(btn_save, btn_restart)
        mess = bot.send_message(user_id,
                                'Пожалуйста, выберите один из предложенных вариантов (Сохранить, Заполнить заново)',
                                reply_markup=markup)
        bot.register_next_step_handler(mess, process_save_step)
